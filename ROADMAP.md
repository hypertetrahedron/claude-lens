...
| Container-mounted remote roots | Complete | A Docker host-mount puts the container's `~/.claude` at `~/.claude-<project>/data` on the host, so `projects/` sits one level below the `.claude*` match that `find -prune` stops at - those hosts were skipped silently. The remote script now globs one level down (no recursive descent, same cost) and each mount is labeled after its `.claude-<project>` folder rather than the shared `data`. Reported working on 2026-09-02 against a host running several such mounts (all ingesting under distinct per-project labels); covered here by a POSIX-sh test that runs the real script over a fake `$HOME` (skipped on Windows). |
...
| Project-run highlighting | Complete | Sorting by project marks the first row of each project's run (rule above in the project's colour, name un-muted), so consecutive rows from one project are separable. Applied only under a project sort - elsewhere it would draw a division that does not exist. In grouped mode each header row is tinted with the same colour plus a bar down its left edge. Eight hues taken in turn and cycled: a boundary marker, not an identity, so two distant blocks can share one but neighbours never do. Palette validated in both themes (worst adjacent pair dE 9.1 light / 8.4 dark under simulated protanopia, 19.6 / 19.3 with normal vision); a browser without `color-mix` falls back to the old single wash. Checked in Chrome, light and dark, against the real payload. |
